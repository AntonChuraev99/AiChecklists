...
- Action buttons only for **non-obvious** features. Allowed deeplinks: `gisti://widget_instruction`, `gisti://create?viewMode=weekly`. Both pass the "non-obvious entry" test — widget setup happens in the launcher, weekly creation otherwise needs Edit-mode toggling. CTAs pointing at plain home/analyze/templates/create duplicate the bottom nav and drawer — **do not add them**.
