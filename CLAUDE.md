...
- **Already gitignored (NOT in public):** `commonMain/` (stubs), `SECURITY.md`, `hosting/.firebase/`, `prompts_private.py`, `.firebaserc`, `extensions/*.env`, `graphify-out/`, `claude_design/`, `google_play_translate/`, `.claude/` except `rules/` — each ships a tracked `*.example` where a template helps. ⚠️ `.claude/skills/` stays out because it holds verbatim end-user chat messages; `.claude/hooks/` because it hardcodes a machine-local checkout path.
- **Docs are published by whitelist, numbers never (since 2026-08-18).** `docs/` used to be ignored wholesale, so a task's documentation could not ride in its own MR. Now the task-shaped directories (`solutions` · `decisions` · `active` · `completed` · `plans` · `todos` · `backlog` · `work` · `guidelines` · `reference` · `brainstorms` · `archive` · `designs` · `redesign` · `roadmap`) are tracked as **`.md` only** — every binary stays out, because 311 MB of video and screenshots would sit in history forever. **What a doc describes is publishable; what it measures is not.** No revenue, MRR, ARPU, LTV, payback, purchase/subscriber/install counts, conversion or retention percentages, A/B arm results, audience sizes, crash-free rates, ad-account ids, private dashboard links or third-party identities. Enforced by `scripts/docs_leak_scan.py` — pre-commit locally, `.github/workflows/docs-leak-guard.yml` in CI (CI is the binding one; the hook is absent on a fresh clone). Permanently ignored: `docs/marketing/`, `docs/reports/`, `docs/healthcheck/`, `docs/PRODUCT.md`, `docs/product-features.md`, `docs/unit-economics.md`, `docs/pricing-strategy.md`. **Adding a directory to the whitelist is not a review** — it only means the scanner now watches it; a regex cannot read a sentence, so a number phrased without its keyword ("the funnel loses a third here") still needs your eyes.
...
**Pre-deploy: ALWAYS verify project ↔ account link FIRST (mandatory, 2026-07-01).** Before ANY `wrangler deploy`/`versions upload`, run `npx wrangler@4 whoami` and confirm the login owns the target zone/worker. The `gisti-ai.com` zone + `checklists` worker live in the owner's **personal** CF account (workers.dev subdomain `gisti`; the account id is the one in `wrangler.jsonc`) — the local wrangler is often still logged into a **different, work** account used for an unrelated project, which has neither the zone nor the worker. Wrong login → the script uploads into the wrong account and the route fails (`Could not find zone` / `register a workers.dev subdomain`). Confirm the config's `account_id` matches the intended account before deploying; if `whoami` shows the work account, STOP and have the user re-auth (`wrangler login`) or supply a `CLOUDFLARE_API_TOKEN` scoped to the personal account. Precedent: a probe deploy landed a throwaway `gisti-app-stage` worker in both work accounts before erroring on the zone (deleted after). Exact addresses and account ids: `~/.claude/config/project-credentials.local.md` (never in this repo). Details: project memory `cloudflare-deploy-accounts-and-workers-dev-default`.
...
**Config has FOUR layers** — check all before concluding: Firebase RC client template · server template `firebase-server` (all percentage conditions) · Firestore `remote_config/current` (credits) · web `init.js` (drifts). Mandatory measurement filters: `products_load_failed = 0` (strips review emulators; without it the RC-fail share reads an order of magnitude high) and `rc_activated = True`; compare **raw** shares across runs, never cleaned ones. **No new revenue-objective experiments** at the current purchase volume ([ADR 07-15]). Keep the RC-limits table in `docs/unit-economics.md` in sync too. Both docs are gitignored (local-only) — they hold the numbers, this file does not. Prod truth = Firebase Console (RC + A/B Tests) + Amplitude (project id in `docs/PRODUCT.md`). <!-- docs-leak-scan: reviewed — no figure here, only where the figures live -->
...
**Play Store listing — source of truth: `docs/store-screenshots/store-listing-en.md`** (tracked; the rest of `docs/store-screenshots/` is screenshots and stays ignored). Any listing change starts in this doc (title/short/full with char limits 30/80/4000, keyword-coverage table), then is pasted into Play Console → Main store listing — and any console edit must be mirrored back, otherwise the doc silently drifts from prod (this happened before the 2026-07-02 ASO audit).
...
- Phase 0 foundation LIVE: `robots.txt` + `sitemap.xml` (en + hi, verified 2026-08-03) + IndexNow, all serving; GSC domain property verified. Pages **are** indexed — the gap is **ranking / domain authority**, not infra or indexing. Current GSC numbers (impressions, average position, backlink count) + owner actions live in the SEO plan's verification block, which stays gitignored.
...
|---|---|---|---|
| AI requests/day | 10 | 300 | `RemoteConfigKeys.kt:75-76` | <!-- docs-leak-scan: reviewed — published Free/Premium limits, visible in-app -->
...
| `ai-chat` | `feature/aichat/**`, `firebase-functions/**` — tier routing, FEATURE_CATALOG, TDD rule | <!-- docs-leak-scan: reviewed — constant name in a rules index -->
...
For wasmJs interop, the reference project is the owner's other KMP repo, checked out as a sibling of this one (path in `~/.claude/config/project-credentials.local.md`).
