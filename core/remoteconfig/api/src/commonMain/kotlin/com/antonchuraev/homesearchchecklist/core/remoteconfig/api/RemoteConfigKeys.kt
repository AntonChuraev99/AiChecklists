...
object RemoteConfigDefaults {
    ...
    // Update Feed — 13 chronological feature posts (v1.6–v1.11, Jan–Mar 2026) + 6 de-duped Google
    // Play release notes (v1.6–v1.11 only). Posts use main-version format (X.Y). Grouped by version
    // in the UI (Google Play release-notes style). 6 unique post-versions → 6 release cards.
    // De-dup rules applied: each storeDescription line appears exactly once across all versions.
    // v1.8, v1.9, v1.11 have posts but no storeDescription (all lines were covered by earlier
    // versions or by feature posts in the same group). v1.12 omitted entirely (no unique content).
