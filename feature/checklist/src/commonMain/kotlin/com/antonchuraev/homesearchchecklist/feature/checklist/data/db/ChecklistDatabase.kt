...
val MIGRATION_12_13 = object : Migration(12, 13) {
    override suspend fun migrate(connection: SQLiteConnection) {
        // Add linkedChecklistId to ai_chat_history so "Open checklist" deeplink survives
        // navigation away/back. Existing rows have no linked checklist — null is correct.
        connection.execSQL("ALTER TABLE ai_chat_history ADD COLUMN linkedChecklistId INTEGER DEFAULT NULL")
    }
}
...
val MIGRATION_15_16 = object : Migration(15, 16) {
    override suspend fun migrate(connection: SQLiteConnection) {
        // Add foldersEnabled column; existing rows default to 0 (folders feature off)
        connection.execSQL("ALTER TABLE checklists ADD COLUMN foldersEnabled INTEGER NOT NULL DEFAULT 0")
    }
}

val MIGRATION_16_17 = object : Migration(16, 17) {
    override suspend fun migrate(connection: SQLiteConnection) {
        // Add reminderFullScreen column; existing rows default to 0 (alarm-style FSI off)
        connection.execSQL("ALTER TABLE checklists ADD COLUMN reminderFullScreen INTEGER NOT NULL DEFAULT 0")
    }
}
